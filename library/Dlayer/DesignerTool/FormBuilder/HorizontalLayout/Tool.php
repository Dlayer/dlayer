...
/**
 * Horizontal layout tool
 *
 * @author Dean Blackborough <dean@g3d-development.com>
 * @copyright G3D Development Limited
 * @license https://github.com/Dlayer/dlayer/blob/master/LICENSE
 */
class Dlayer_DesignerTool_FormBuilder_HorizontalLayout_Tool extends Dlayer_Tool_Form
{
    /**
    ...
    {
        $valid = false;
        if (array_key_exists('horizontal', $params) === true && array_key_exists('label_width', $params) === true &&
            array_key_exists('element_width', $params) === true) {
    ...
    protected function paramsValid(array $params)
    {
        ...
        if (intval($params['horizontal']) > 0 && intval($params['label_width']) > 0 &&
            intval($params['element_width']) > 0 && $model->validId(intval($params['horizontal'])) === true) {
    ...
    protected function paramsAssignAuto(array $params)
    {
        $this->params_auto = array(
            'layout' => intval($params['horizontal']),
            'label_width' => intval($params['label_width']),
            'element_width' => intval($params['element_width'])
        );
    }
    ...
    {
        $model = new Dlayer_DesignerTool_FormBuilder_HorizontalLayout_Model();
        $result = $model->setLayout(
            $this->site_id,
            $this->form_id,
            $this->params_auto['layout'],
            $this->params_auto['label_width'],
            $this->params_auto['element_width']
        );
