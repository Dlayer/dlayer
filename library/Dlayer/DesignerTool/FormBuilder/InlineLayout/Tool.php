 ...
 */
class Dlayer_DesignerTool_FormBuilder_InlineLayout_Tool extends Dlayer_Tool_Form
{
    /**
    ...
    {
        $valid = false;
        if (array_key_exists('inline', $params) === true) {
            $valid = true;
        }
    ...
    protected function paramsValid(array $params)
    {
        ...
        if (intval($params['inline']) > 0 && $model->validId(intval($params['inline'])) === true) {
            $valid = true;
        }
