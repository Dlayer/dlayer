 ...
 */
class Dlayer_DesignerTool_ContentManager_Html_Model extends Zend_Db_Table_Abstract
{
    /**
    ...
    {
        $sql = "SELECT data_id
				FROM user_site_page_content_item_html 
    ...
    {
        $sql = "UPDATE user_site_content_html 
        ...
        if(array_key_exists('instances', $params) === TRUE)
        {
            if($params['instances'] === 1)
            {
                ...
                else
                {
                    ...
                    {
                        throw new Exception('Error updating data id for html content items using data id: ' .
                            $current_data_id);
                    }
    ...
    {
        $sql = "UPDATE user_site_page_content_item_html 
    ...
    {
        $sql = "DELETE FROM user_site_content_html 
                WHERE site_id = :site_id 
                AND id = :delete_id";
        $stmt = $this->_db->prepare($sql);
        $stmt->bindValue(':site_id', $site_id, PDO::PARAM_INT);
