 ...
 * @category Model
 */
class Dlayer_Model_Admin_Site extends Zend_Db_Table_Abstract
{
    /**
    ...
    public function lastAccessedSite($identity_id)
    {
        $sql = "SELECT 
                    ...
                    `ush`.`site_id`
				FROM 
				    `user_site_history` `ush`
				INNER JOIN 
				    `user_site` `us` ON 
				        `ush`.`site_id` = `us`.`id`
				WHERE 
				    `us`.`identity_id` = :identity_id";
        $stmt = $this->_db->prepare($sql);
        $stmt->bindValue(':identity_id', $identity_id, PDO::PARAM_INT);
        $stmt->execute();
        ...
        if ($result !== false) {
            ...
            if ($result !== false) {
                ...
                $site_id = $model_sites->saveSite($name, $identity_id);
