...
* @version $Id: Content.php 1306 2013-12-01 12:21:22Z Dean.Blackborough $
*/
...
{   
    ...
    public function baseFontFamily($site_id) 
    {
        $sql = "SELECT dcff.id, dcff.css, dcff.`name` 
                ...
                JOIN dlayer_modules dm ON usff.module_id = dm.id 
                    AND dm.enabled = 1 
                JOIN designer_css_font_families dcff 
    ...
    */
    ...
    {
        $sql = "UPDATE user_settings_headings 
    ...
    */
    ...
    {
        $sql = "UPDATE user_settings_font_families 
                SET font_family_id = :font_family_id 
                WHERE site_id = :site_id 
                AND module_id = (SELECT id FROM dlayer_modules 
                WHERE `name` = :module) 
                LIMIT 1";
        $stmt = $this->_db->prepare($sql);
        $stmt->bindValue(':site_id', $site_id, PDO::PARAM_INT);
