...
* @version $Id: Settings.php 1321 2013-12-04 02:13:43Z Dean.Blackborough $
* @category View model
*/
    ...
    */
    public function headingStyles($site_id)
    {
        $sql = "SELECT dch.tag, dcts.css AS style, dctw.css AS weight,
                ...
                FROM user_settings_headings ush
                JOIN designer_content_headings dch ON ush.heading_id = dch.id
                JOIN designer_css_text_styles dcts ON ush.style_id = dcts.id
                JOIN designer_css_text_weights dctw ON ush.weight_id = dctw.id
                JOIN designer_css_text_decorations dctd ON ush.decoration_id = dctd.id
    ...
    */
    public function baseFontFamily($site_id, $module)
    {
        $sql = "SELECT dcff.css AS font_family
                FROM user_settings_font_families usff
                JOIN dlayer_modules dm ON usff.module_id = dm.id
                JOIN designer_css_font_families dcff ON usff.font_family_id = dcff.id
