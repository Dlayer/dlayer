...
class Dlayer_Model_Settings extends Zend_Db_Table_Abstract
{
    ...
    public function setDefaultColourPaletteColors($site_id)
    {
        $palettes = array(
            1 => array(
                ...
                array('type' => 2, 'name' => 'Dark grey', 'hex' => '#333333'),
            ...
            2 => array(
                ...
                array('type' => 2, 'name' => 'Green', 'hex' => '#5cb85c'),
        ...
        foreach ($palettes as $i => $palette) {
            ...
            $stmt_palette->bindValue(':view_script', 'palette-' . $i, PDO::PARAM_STR);
            $stmt_palette->execute();

            $result = $stmt_palette->fetch();

            $palette_id = intval($result['id']);
            ...
            foreach ($palette as $color) {
                ...
                $stmt->bindValue(':color_type_id', $color['type'], PDO::PARAM_INT);
    ...
    public function setDefaultBaseFontFamilies($site_id)
    {
        ...
        $stmt->bindValue(':text_weight_id', 1, PDO::PARAM_INT);
