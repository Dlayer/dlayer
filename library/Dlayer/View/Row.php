    ...
    public function render()
    {
        ...
        if (array_key_exists($this->column_id, $this->rows) === true) {
            foreach ($this->rows[$this->column_id] as $row) {
                ...
                if ($show_toggle === true) {
                    $html .= '<div style="position: relative"><button type="button" title="Toggle row" class="btn btn-info btn-xs toggle-row" data-row-id="' . $this->view->escape($row['id']) . '" style="position: absolute; top: 2px; right: 2px; z-index: 1000;"><span class="toggle-icon-' . $this->view->escape($row['id']) . ' glyphicon glyphicon-menu-up" aria-hidden="true"></span></button></div>';
                }
                $html .= $content;
                $html .= '</div>';
            }
